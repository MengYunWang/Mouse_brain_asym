...
if (roi=="AUD") {
  write.xlsx(data2save, file = "output/1_Gene_expression/All_genes/Gene_by_gene/region_seperated/batch1_2/Sex_data2analysis.xlsx", sheetName = roi)
} else {
  wb <- loadWorkbook("output/1_Gene_expression/All_genes/Gene_by_gene/region_seperated/batch1_2/Sex_data2analysis.xlsx")
  if (roi %in% names(wb)) {
    removeWorksheet(wb, roi)  # Remove the existing sheet
  }
  addWorksheet(wb, roi)
  writeData(wb, roi, data2save)
  saveWorkbook(wb, "output/1_Gene_expression/All_genes/Gene_by_gene/region_seperated/batch1_2/Sex_data2analysis.xlsx", overwrite=TRUE)
}
