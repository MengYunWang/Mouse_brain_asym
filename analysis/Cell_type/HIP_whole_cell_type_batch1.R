...
coordi_list <- lapply(region, process_coordi)
coordi <- do.call(rbind, coordi_list)
...
# Try using a convex hull to reorder points
coords <- as.matrix(coordi[, c("x", "y")])

concave_hull <- concaveman(coords, concavity = 1.8)  # Adjust concavity parameter if needed
concave_hull_df <- data.frame(x = concave_hull[, 1], y = concave_hull[, 2])
...
# Create the polygon
HIP_to_crop <- create_roi_polygon(concave_hull_df)
...
lookup <- list("M669" = "b174",
               ...
               "M678" = "b442",
               "F679" = "b442",
               "M680" = "b442", #attention here, F680 is lablled as M680
...
if(hemi=="left"){
  plot2save <- paste0("output/2_Cell_type/acr_sample/batch1_2/HIP/Plot_", mouse_id, "_lhip_cell_type_", file_name, ".png")
} else {
  plot2save <- paste0("output/2_Cell_type/acr_sample/batch1_2/HIP/Plot_", mouse_id, "_rhip_cell_type_", file_name, ".png")
}
...
library(openxlsx)
if(hemi=="left"){
  cell_type2save <- paste("output/2_Cell_type/acr_sample/batch1_2/HIP/", mouse_id, "_hip_cell_type_", file_name, ".xlsx", sep = "")
